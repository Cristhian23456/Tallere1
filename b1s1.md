...
$8 ==> [Tuple[x=1, y=1], Tuple[x=2, y=4], Tuple[x=3, y=9], Tuple[x=4, y=16], Tuple[x=5, y=25]]
$9 ==> [2, 4]
$10 ==> 120
```
**En Scala (scala)**
...
scala-cli repl
```
Seginda parte. Codigo pasado en Haskell, Python, Scala3.
0Haskell
```haskell
getAverage :: [Int] -> Double
getAverage nums = 
    -- Use the built-in 'sum' to get the total
    let totalSum = sum nums
        -- Use 'genericLength' to get the size as a floating-point number
        listLength = genericLength nums
    in
        -- 'fromIntegral' converts the integer sum to a floating-point type (Double)
        fromIntegral totalSum / listLength
```
Scala
```scala
def getAverage(nums: List[Int]): Double = {
  // Use pattern matching to handle the empty list case immutably
  nums match {
    case Nil => 0.0 // Return 0.0 for an empty list
    case list => 
      // 1. Use 'foldLeft' (a form of 'reduce') to calculate the sum immutably
      //    We start the accumulator at 0.0 (Double)
      val totalSum = list.foldLeft(0.0) { (accumulator, num) =>
        accumulator + num
      }
      
      // 2. Calculate the average using immutable values
      totalSum / list.size.toDouble
  }
}

// Alternative (more concise) using built-in functions:
// def getAverage(nums: List[Int]): Double = 
//   if (nums.isEmpty) 0.0 else nums.map(_.toDouble).sum / nums.size
```
Python 
```python
from functools import reduce
...
def get_average(nums: list[int]) -> float:
    # Handle the empty list case immutably
    if not nums:
        return 0.0

    # 1. Use 'reduce' (equivalent to 'fold' in functional languages) to calculate the sum.
    #    The initial value 0.0 ensures the accumulator remains a float.
    total_sum = reduce(lambda acc, x: acc + x, nums, 0.0)
    
    # 2. Calculate the average using the immutable result
    return total_sum / len(nums)

# Alternative (more idiomatic Python using built-in functional components):
# def get_average_idiomatic(nums: list[int]) -> float:
#     return sum(nums) / len(nums) if nums else 0.0
```
Esta segunda partval nums = List(1, 2, 3, 4, 5)
nums.map(_ * 10)                                                                              
nums.map(n => (n, n * n))
nums.filter(n => n % 2 == 0)
nums.reduce((n1, n2) => n1 * n2)e finaliza cuando muestre a su docente todo lo implementado.
